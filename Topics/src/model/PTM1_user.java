...
public class PTM1_user {
	// Aadi: Similar to PTM1.java. But then what changed here? While estimating the distribution for z | w, u d , 
	// this program does not incorporate user specific distributions. Ok, so?
			...
			for (int u=0; u<U; u++) {  // user u
				for (int i=0; i<w_uij[u].length; i++) { // query i
					...
					for (int j=0; j<w_uij[u][i].length; j++) { // position j
						...
						// calculate distribution p(z|w,u,d) \propto p(w,u,z|d) = p(w|z)p(u|z)p(z|d)
