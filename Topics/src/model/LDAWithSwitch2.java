 ...
 */
public class LDAWithSwitch2 {

	public int[] Z; // number of topics in each distribution. Z.length = total number of alternate distributions
	...
	public double[][] b_sz; // term smoothing hyperparameter \beta 
	...
	public double[][][] estimate(int[][] w_di, int W, int D, long N, int S, int[] Z, double[] a, double[] b, double g, int burnIn, int samples, int step) {
		...
		for (int i = 0; i < S; i++)
		{	
			int num_of_topics = Z[i];
			b_sz[i] = new double[num_of_topics];
			
			for (int j = 0; j < num_of_topics; j++)
			{
				b_sz[i][j] = b[i]/num_of_topics;
			}
		...
		{	
			...
			a_sz[i] = new double[num_of_topics];
			
			for (int j = 0; j < num_of_topics; j++)
			{
				a_sz[i][j] = a[i]/num_of_topics;
			}
