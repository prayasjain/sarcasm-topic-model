			...
			for (int u=0; u<w_uij.length; u++) {  // user u
				for (int i=0; i<w_uij[u].length; i++) { // query i
					...
					for (int j=0; j<w_uij[u][i].length; j++) { // position j
						...
						// calculate distribution: P(x,y,z|w,u,d) \propto P(w,x,y,z|u,d) = P(w|x)P(x|,y,z)P(y|u)P(z|d)
						// This is not pretty!!
						...
						for (x=0; x<X; x++) {
							for (y=0; y<Y; y++) {
								for (z=0; z<Z; z++) {
									p[x][y][z] = ( (N_wx[w][x] + b_on_W)/(N_x[x] + b) ) * ( (N_xyz[x][y][z] + bx_on_X)/(N_yz[y][z] + bx) ) * (N_yu[y][u] + au_on_Y) * (N_zd[z][d] + ad_on_Z);
									total += p[x][y][z];
								}
							}
						}
