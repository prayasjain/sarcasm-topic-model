...
/*
 * Program not working correctly. Must not be run. 24th October, 2013.
		...
		for (int iteration=0; iteration<burnIn+samples; iteration++) {    // Aadi: For burn-in + samples number of iterations
			...
			for (int d=0; d<D; d++) { // document d
				...
				for (int i=0; i<w_di[d].length; ) { 
					System.out.println("Starting with position: "+i);
					int this_sent_i = i;
					...
					System.out.println("start decrement on word number "+(this_sent_i - i));
					do
					{
					...
					}while (this_sent_i < w_di[d].length && !l_w[w_di[d][this_sent_i]].equals("BR"));
					// position i
														// Aadi: Go over each word of all documents
					System.out.println("stop on word number "+(this_sent_i -i ));
					// Now, resample s
					...
					for (s=0; s<S; s++) {
						...
						if (p[s] < 0){
							System.out.println("Hello boss! Paagal ho kya?");
						}
					...
					this_sent_i = i;
					System.out.println("start increment#1 on word number "+(this_sent_i - i));
					do
					{
						w = w_di[d][this_sent_i];
					...
					System.out.println("stop increment#1 on word number "+(this_sent_i - i));
					...
					this_sent_i = i;
					System.out.println("start increment#2 on word number "+(this_sent_i - i));
					do
					{
						w = w_di[d][this_sent_i];
					...
					System.out.println("stop increment#2 on word number "+(this_sent_i - i));
					i = this_sent_i + 1;
			...
			System.out.println("Check if you want to burn-in");
			...
			System.out.println("Re-estimate hyperparameters");
			do{
				...
				for (int s=0; s<S; s++) for (int i = 0; i < Z; i++) alphasum += a_sz[s][i];
				 ...
				 */
				System.out.println("Estimate log-likelihood.");
				double ll = 0.0d;
		...
		for (int s=0; s<S; s++) { 
			for (int d=0; d<D; d++) { // document d
				for (int i=0; i<N_d[d]; i++) { // position i
					...
					ll += Math.log( (N_swz[s][w][z] + b_s[s]/b)/(N_sz[s][z] + b) ); 
