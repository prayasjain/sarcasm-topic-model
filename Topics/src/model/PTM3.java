...
public class PTM3 {
	...
	public double[][] estimate(int[][][] w_uij, int[][] d_ui, int W, int D, long N, int Y, int Z, double au, double ad, double b, int burnin, int samples, int step) {
		...
		for (int iteration=0; iteration<(burnin+samples); iteration++) {
			for (int u=0; u<w_uij.length; u++) {  // user u
				for (int i=0; i<w_uij[u].length; i++) { // query i
					...
					for (int j=0; j<w_uij[u][i].length; j++) { // position j
						...
						// calculate distribution:  p(y,z|w,u,d) \propto p(w,y,z|u,d) = p(w|y,z)p(y|u)p(z|d)
						double[][] p = new double[Y][Z];
						double total = 0;
						for (y=0; y<Y; y++) 
							for (z=0; z<Z; z++) { 
								p[y][z] = ( (N_wyz[w][y][z] + b_on_W)/(N_yz[y][z] + b) ) * (N_yu[y][u] + au_on_Y) * (N_zd[z][d] + ad_on_Z);
								total += p[y][z];
							}
			...
			if (iteration%step==0) System.out.println("PTM3: iteration: "+iteration+", log-likelihood: "+logLikelihood());
		}
