 ...
 * This is done only during initialization. Sampling unchanged from LDA2.
 */
/*
 * Includes hyperparameter estimation
 */
public class LDAWithSentiInit {
	...
	public double[][] estimate(int[][] w_di, int W, int D, long N, int Z, HashMap hm_senti, String[] l_w, double a, double b, int burnIn, int samples, int step) {
		...
		for (int d=0; d<D; d++) {  // document d
			...
			for (int i=0; i<I; i++) { // position i
				 ...
				 * between 0 and z.
				 */
				if (hm_senti.containsKey(l_w[w_di[d][i]]))
				{
					String polarity = (String) hm_senti.get(l_w[w_di[d][i]]);
					if (polarity.equals("+1"))
					{
						z = (Z/3) + (int)(Math.random() * (((2*Z/3 -1) - (Z/3)) + 1)); 
						//			System.out.println("Word "+ l_w[w_di[d][i]] + " is positive. gets initialized with topic : "+ z);
					}
					else
					{
		...
		for (int iteration=0; iteration<burnIn+samples; iteration++) {    // Aadi: For burn-in + samples number of iterations
			for (int d=0; d<D; d++) { // document d
				for (int i=0; i<w_di[d].length; i++) { // position i
					...
					double total = 0;
					for (z=0; z<Z; z++) {
						p[z] = ( (N_wz[w][z] + b_on_W)/(N_z[z] + b) ) * (N_zd[z][d] + a_z[z]); /* HE */
						total += p[z];
					}
			...
			if (iteration >= burnIn) {	
				...
				for (int d=0; d<D; d++) for (int z=0; z<Z; z++) P_z_d[z][d] += (N_zd[z][d] + + a_z[z])/(N_d[d] + a);  /* HE */
			}
