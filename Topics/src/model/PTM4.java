...
public class PTM4 {
	...
	public double[][] estimate(int[][][] w_uij, int[][] d_ui, int W, int D, long N, int X, int Y, int Z, double au, double ad, double bx, double b, int burnin, int samples, int step) {
		...
		for (int iteration=0; iteration<(burnin+samples); iteration++) {
			for (int u=0; u<w_uij.length; u++) {  // user u
				for (int i=0; i<w_uij[u].length; i++) { // query i
					...
					for (int j=0; j<w_uij[u][i].length; j++) { // position j
						...
						// calculate distribution: P(y,z|x,u,d) \propto P(x,y,z|u,d) = P(x|,y,z)P(y|u)P(z|d)
						double[][] p = new double[Y][Z];
						double total = 0;
						for (y=0; y<Y; y++) {
							for (z=0; z<Z; z++) {
								p[y][z] = ( (N_xyz[x][y][z] + bx_on_X)/(N_yz[y][z] + bx) ) * (N_yu[y][u] + au_on_Y) * (N_zd[z][d] + ad_on_Z);
								total += p[y][z];
							}
						}
						// resample 
						...
						// calculate distribution: P(x|w,y,z) \propto P(w,x|y,z) = P(w|x)P(x|,y,z)
						double[] p2 = new double[X];
						total = 0;
						for (x=0; x<X; x++) {
							p2[x] = ( (N_wx[w][x] + b_on_W)/(N_x[x] + b) ) * (N_xyz[x][y][z] + bx_on_X);
							total += p2[x];
						}
						// resample 
			...
			if (iteration%step==0) System.out.println("PTM4: iteration: "+iteration+", log-likelihood: "+logLikelihood());
		}
