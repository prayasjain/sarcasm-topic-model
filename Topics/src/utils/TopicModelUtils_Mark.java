...
public class TopicModelUtils_Mark {
	...
	public static void printTopTerms(double[][] p_w_z, String[] lexicon) {
		...
		for (int z=0; z<Z; z++) {
			...
			for (int i=0; i<((20>rankedListOfTerms.length)?rankedListOfTerms.length:20); i++) {
				int w = rankedListOfTerms[i];
				System.out.println(p_w[w] +" "+ lexicon[w]);
			}
		}
	}
		...
		for (int z=0; z<Z; z++) {
			...
			for (int i=0; i<20; i++) {
				...
				{	
					...
					else if ((Integer) hm_senti.get(lexicon[w]) == -1)
						neg_count++;
				}
			}
