...
public class QueryTokenizer {
	...
	public QueryTokenizer(boolean stop, boolean stem, boolean removePunctuation) {
		this.stop = stop;
		this.stem = stem;
		
		if (stop) stopwords = getStopwords();
		if (stem) stemmer = new Porter();
		if (removePunctuation) regex = "(\\s|(?!_)\\p{punct})+";
	}
	...
	public ArrayList<String> tokenize(String s) {
		...
		while(st.hasMoreTokens())
		{
			...
			if(!(curr.startsWith("@")||curr.startsWith("http://")))
				s2=s2+curr+" ";
		}
	...
	private HashSet<String> getStopwords() {
		...
		for (String s: month) stopwords.add(s);
		return stopwords;
